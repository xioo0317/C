...
// ===========================================================================
// JSON serialization (enum -> string helpers have internal linkage so they
// never collide with anything in main.cpp)
// ===========================================================================
...
const char* ksu_priv_name(KsuPrivLevel p) {
    switch (p) {
...
const char* magisk_priv_name(MagiskPrivLevel p) {
    switch (p) {
...
const char* susfs_abi_name(SusfsAbi a) {
    switch (a) {
        case SusfsAbi::None:     return "none";
        case SusfsAbi::Prctl:    return "prctl";
        case SusfsAbi::RebootV2: return "reboot_v2";
    }
    return "unknown";
}
...
std::string result_to_json_string(const DetectResult& r) {
    ...
    // --- KernelSU ---
    {
        ...
        if (r.ksu.present) {
            ...
            k["priv_level"] = ksu_priv_name(r.ksu.priv_level);
    ...
    // --- Magisk ---
    {
        ...
        m["priv_level"] = magisk_priv_name(r.magisk.priv_level);
    ...
    // --- SusFS ---
    {
        ...
        if (r.susfs.detected) {
            ...
            s["abi"] = susfs_abi_name(r.susfs.abi);
    ...
    {
        json b;
        b["detected"] = r.jailbreak.detected;
        b["indicators"] = r.jailbreak.indicators;
        j["jailbreak"] = std::move(b);
    }
